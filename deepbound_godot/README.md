...
This build covers playable Sprints 1-5 plus the first template-backed settlement pass: Band 1 foundation, Band 1 polish/UI, first outpost beacon, Band 2 ant chamber hooks, Band 3 pyramid hooks, swept collision, modular sprite animation, block-backed chest containers, grid inventory, extra-slot hotbar placement, physical world drops, click pickup, special auto-pickup, the heart HUD, cached terrain rendering, the main menu/pause menu, single-slot save/load, the prefab/template designer utility, an imported Band 1 goblin village prefab, and Band 2 dwarf fortress and settlement prefabs.

## Gameplay Docs

The current player/system guide is in `docs/Gameplay.md`. It covers controls, `I` inventory, `Escape` pause/save/load, hotbar selection with `1-6` and mouse wheel, right-click chest use and placement, dual inventory panels, drag/drop stack rules, physical world drops, click pickup, special auto-pickup, heart HP rules, and the HUD.
The prefab/template workflow is in `docs/Prefab_Template_System.md`, including the fixed clipped designer canvas, visible scrollbars, undo/redo, and save-stable template integration.
...
The dwarf fortress and dwarf settlement kits are documented in the Art Production package and generated by `tools/build_pixel_assets.py`.
